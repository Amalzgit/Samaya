...
const updateCart = async (req, res) => {
  try {
    ...
    // Find the cart for the current user and update the quantity of the specific product
    ...
    const formattedSubtotal = formatCurrency(
      (parseFloat(
        cartItems.find((item) => item.product._id.toString() === productId)
          ?.product.price
      ) || 0) * quantity
    );
    ...
    res.json({
      ...
      formattedTotalPrice,
      formattedDiscountedTotal,
      isCouponApplied : cart.appliedCoupon ? true : false,
      discountAmount
    });
  } catch (error) {
...
const applyCoupon = async (req, res) => {
	try {
		...
		if (cart.appliedCoupon) {
			...
			if (
				existingCoupon &&
				checkCouponRemoval(existingCoupon, cartTotal)
			) {
		...
		if (usedCoupons.includes(coupon._id.toString())) {
			return res
				.status(400)
				.json({ message: "Coupon already used once" });
		}
